...
public enum ContractsTrust {
    ...
    public static let enforceSignatures = false
...
public struct Ed25519DetachedSignatureVerifier: Sendable {
    ...
    public static let bundled = Ed25519DetachedSignatureVerifier(publicKey: ContractsTrust.bundledPublicKey)

    /// Verify a detached `signature` over the exact `data` bytes.
    /// Returns `false` if no public key is configured.
    public func isValid(signature: Data, for data: Data) -> Bool {
        guard let publicKey else { return false }
        return publicKey.isValidSignature(signature, for: data)
    }
}

public enum SignedAssetVerificationError: Error, Sendable {
...
public enum SignedAsset {
    ...
    /// crashing.
    public static func decodeSignature(_ data: Data) -> Data {
