...
public actor SwiftDataGroupStore: GroupStore {
    ...
    public static func inMemory() -> SwiftDataGroupStore {
        let schema = Schema([PersistedGroup.self])
    ...
    public func markRead(id: String, ownerIDString: String, at date: Date) {
        let descriptor = FetchDescriptor<PersistedGroup>(
            predicate: #Predicate { $0.id == id && $0.ownerIdentityIDString == ownerIDString }
        )
        guard let row = try? context.fetch(descriptor).first else { return }
    ...
    public func delete(id: String, ownerIDString: String) {
        let descriptor = FetchDescriptor<PersistedGroup>(
            predicate: #Predicate { $0.id == id && $0.ownerIdentityIDString == ownerIDString }
        )
