...
public enum CreateGroupRoute: Equatable, Sendable {
...
public struct OnymInvitee: Equatable, Identifiable, Sendable {
    public let id: UUID
    public let inboxPublicKey: Data  // 32 bytes
...
@MainActor
@Observable
public final class CreateGroupFlow {
    // MARK: - Form state
    ...
    /// retype.
    public private(set) var generatedName: String
    ...
    public var inviteeInput: String = ""
    /// Inline error shown under the InviteByKey TextField. Cleared on
    /// every keystroke.
    public internal(set) var inviteeError: String?

    // MARK: - Navigation + pipeline

    public internal(set) var route: CreateGroupRoute = .step1
    ...
    public internal(set) var error: CreateGroupError?
    ...
    public init(interactor: CreateGroupInteractor) {
    ...
    public func tappedNameFieldFocused() {
    ...
    public var canAdvanceToStep2: Bool { governance.isAvailable }
    ...
    public func removeInvitee(at index: Int) {
    ...
    public var createCTALabel: String {
        switch governance {
        case .oneOnOne:
    ...
    public var canAddMoreInvitees: Bool {
        switch governance {
    ...
    /// paste.
    public func tappedScannedKey(_ raw: String) {
    ...
    public static func canonicalizeInviteKey(_ raw: String) -> String {
    ...
    /// design's `(43/64)` chip.
    public var inviteeInputCleanedLength: Int {
        inviteeInput.replacingOccurrences(of: "\\s+", with: "", options: .regularExpression).count
    }

    public var inviteeInputIsValid: Bool {
        let cleaned = inviteeInput.replacingOccurrences(
            of: "\\s+",
            with: "",
            options: .regularExpression
    ...
    /// the modal and reset.
    public func tappedCancelFromError() {
        reset()
        onClose()
    }
