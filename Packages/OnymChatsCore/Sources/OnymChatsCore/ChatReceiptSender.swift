...
public protocol ChatReceiptSending: Sendable {
    @discardableResult
    func send(
...
public struct ChatReceiptSender: ChatReceiptSending {
...
public enum ReadReceiptsPreference {
    /// Shared with the Settings `@AppStorage` toggle so both read/write
    /// the same key.
    public static let storageKey = "app.onym.ios.chat.sendReadReceipts"

    public internal(set) static var isEnabled: Bool {
        get {
