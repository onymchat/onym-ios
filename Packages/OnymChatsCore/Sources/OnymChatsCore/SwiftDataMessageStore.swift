...
public actor SwiftDataMessageStore: MessageStore {
    ...
    public static func inMemory() -> SwiftDataMessageStore {
        let schema = Schema([PersistedMessage.self])
    ...
    public func latestMessage(groupID: String, ownerIDString: String) -> ChatMessage? {
    ...
    /// returns nothing.
    public func search(ownerIDString: String, query: String, limit: Int = 200) -> [ChatMessage] {
    ...
    @discardableResult
    public func insertOrUpdate(_ message: ChatMessage) -> MessageInsertOutcome {
    ...
    public func needsDeliveredAck(id: UUID, ownerIDString: String) -> Bool {
    ...
    public func markDeliveredAckSent(id: UUID, ownerIDString: String) {
        guard let row = fetchRow(id: id, ownerIDString: ownerIDString) else {
            return
        }
    ...
    public func updateStatus(id: UUID, ownerIDString: String, status: MessageStatus, failureReason: SendFailureReason?) {
        let key = id.uuidString
        let descriptor = FetchDescriptor<PersistedMessage>(
            predicate: #Predicate { $0.id == key && $0.ownerIdentityIDString == ownerIDString }
        )
    ...
    public func delete(id: UUID, ownerIDString: String) {
        let key = id.uuidString
        let descriptor = FetchDescriptor<PersistedMessage>(
            predicate: #Predicate { $0.id == key && $0.ownerIdentityIDString == ownerIDString }
        )
    ...
    public func deleteGroup(groupID: String, ownerIDString: String) {
        let descriptor = FetchDescriptor<PersistedMessage>(
            predicate: #Predicate {
                $0.groupID == groupID && $0.ownerIdentityIDString == ownerIDString
            }
