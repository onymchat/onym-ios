...
public protocol ContractsManifestFetcher: Sendable {
...
public struct GitHubReleasesContractsManifestFetcher: ContractsManifestFetcher {
    /// GitHub redirect that always resolves to the latest release's
    /// `contracts-manifest.json` asset.
    public static let defaultURL = URL(string: "https://github.com/onymchat/onym-contracts/releases/latest/download/contracts-manifest.json")!
    ...
    public func fetchLatest() async throws -> ContractsManifest {
    ...
    /// `network` or `type` doesn't match a known enum value. Exposed
    /// so the same filtering can be tested without going through the
    /// URL pipeline.
    public static func decodeFiltering(_ data: Data) throws -> ContractsManifest {
        let raw: RawManifest
        do {
...
public enum ContractsManifestFetchError: Error, Sendable {
    case badStatus(Int)
