...
public protocol SEPContractTransport: Sendable {
...
public struct SEPContractClient: Sendable {
    ...
    public func createGroupTyranny(_ payload: TyrannyCreateGroupPayload) async throws -> SEPSubmissionResponse {
        try await invoke("create_group", payload: payload, responseType: SEPSubmissionResponse.self)
    }

    public func updateCommitmentTyranny(_ payload: TyrannyUpdateCommitmentPayload) async throws -> SEPSubmissionResponse {
        try await invoke("update_commitment", payload: payload, responseType: SEPSubmissionResponse.self)
    }

    public func createGroupOneOnOne(_ payload: OneOnOneCreateGroupPayload) async throws -> SEPSubmissionResponse {
        try await invoke("create_group", payload: payload, responseType: SEPSubmissionResponse.self)
    }

    public func createGroupAnarchy(_ payload: AnarchyCreateGroupPayload) async throws -> SEPSubmissionResponse {
        try await invoke("create_group", payload: payload, responseType: SEPSubmissionResponse.self)
    }

    public func getCommitment(groupID: Data) async throws -> SEPCommitmentEntry {
        try await invoke(
            "get_commitment",
