...
public final class NostrMessageTransport: MessageTransport {
    ...
    @discardableResult
    public func publish(_ payload: Data, to topic: TransportTopic) async throws -> PublishReceipt {
    ...
    /// Pure event-builder for the publish path. Exposed publicly only
    /// so tests can verify Nostr framing (kind, topic tag, base64
    /// payload, valid id) without standing up a relay.
    public static func buildPublishEvent(
        payload: Data,
    ...
    public func subscribe(topic: TransportTopic, since: Date?) -> AsyncStream<InboundMessage> {
        let sinceUnix: Int64
        if let since {
    ...
    public func unsubscribe(topic: TransportTopic) async {
