...
@MainActor
@Observable
public final class AnchorsPickerFlow {
    public private(set) var state: ContractsState = .empty

    private let repository: ContractsRepository
    private var snapshotTask: Task<Void, Never>?

    public init(repository: ContractsRepository) {
    ...
    public func hasExplicitSelection(for key: AnchorSelectionKey) -> Bool {
    ...
    public func tappedVersion(key: AnchorSelectionKey, releaseTag: String) {
        Task { await repository.select(key: key, releaseTag: releaseTag) }
    }

    public func tappedResetToDefault(key: AnchorSelectionKey) {
