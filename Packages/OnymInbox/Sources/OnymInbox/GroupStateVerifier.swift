...
public protocol GroupStateRefreshing: Sendable {
...
/// Public because it is the default argument of
/// `IncomingMessageDispatcher`'s public initializer (default-argument
/// expressions are emitted at the call site, so everything they
/// reference must be visible to clients).
...
public actor GroupStateVerifier: GroupStateRefreshing {
    ...
    public func handleRefreshRequest(
        _ request: GroupStateRefreshRequest,
        ownerIdentityID: IdentityID,
        requesterEd25519: Data?
    ) async {
